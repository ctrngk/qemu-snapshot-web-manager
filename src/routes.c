                                                 ...
                                                 const char *body)
{
    ...
    if (rc == -2) {
        ...
        snprintf(msg, sizeof(msg),
            ...
            "required afterwards to activate the change."
            " <span title=\"Rebooting from inside the guest only restarts the OS — "
            "the QEMU process keeps its original config. Stop &amp; Start kills the "
            "QEMU process and creates a new one with the latest config.\" "
            "style=\"cursor:help;text-decoration:underline dotted\">"
            "Why not just reboot?</span></small>"
            "<div style=\"margin-top:8px\">"
